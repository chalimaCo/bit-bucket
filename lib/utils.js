...
function serverErrorHandler(err, req, res, next){
    ...
    if (err.errorReport) return res._sendError("intenal server error", err.errorReport);
}
function getAuthToken(user){
    let authToken = jwt.sign({username: user.username}, auth.Tokensecret,{expiresIn: "1h"});
    return authToken
}
...
function ServerError(statusCode, reportMessage, err){
    ...
    if(!err){
        [err, statusCode] = [statusCode, undefined];
    }
